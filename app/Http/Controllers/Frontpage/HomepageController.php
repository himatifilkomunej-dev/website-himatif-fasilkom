...
class HomepageController extends Controller
{
    ...
    public function getHomepage()
    {
        ...
        return view('frontpage.modules.beranda', compact(['header', 'section2', 'section3', 'visionMission', 'prokers',
        'gallery', 'reviews']));
    }
    ...
    public function showBerita($slug)
    {
        ...
        if ($post) {
            return view('frontpage.modules.berita-show', compact(['post', 'otherPosts']));
        } else {
            abort(404);
        }
    }
    ...
    public function showCakap(Request $request)
    {
        ...
        $labelCount = LabelCakap::withCount([
            'cakapKodes' => function ($query) {
        ...
        if ($search_param) {
            ...
            $nim_query
                ->where(function ($query) use ($search_param) {
                    $query->orWhere('nim', 'like', "%$search_param%")->orWhere('name', 'like', "%$search_param%");
                })
                ->limit($limit)
                ->orderBy('nim', 'ASC');
