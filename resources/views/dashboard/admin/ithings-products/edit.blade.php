...
@section('title', 'Edit Produk iThings') {{-- title --}}
...
@section('content') {{-- content --}}
    ...
    {{-- row : form --}}
    <form action="{{ route('dashboard.admin.ithings-products.update', $product->id) }}" enctype="multipart/form-data"
        method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="card">
                    <div class="card-body d-flex flex-column align-items-center">
                        <div class="img-wrapper img-wrapper-upload bg-secondary w-100 h-10rem my-3">
                            <img id="img-product" src="{{ $product->photo ? asset('storage/' . $product->photo) : '' }}"
                                alt="">
                        </div>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        @component('dashboard._components._form-group.input')
                            ...
                            @slot('inputValue', $product->size)
                        @endcomponent

                        {{-- textarea : description --}}
                        <div class="form-group">
                            <label for="input-description">Deskripsi</label>
                            <textarea name="description" id="input-description" class="form-control" rows="4">{{ old('description', $product->description) }}</textarea>
                            @error('description')
                        ...
                        @component('dashboard._components._form-group.input-select')
                            ...
                            @slot('inputValue', $product->category_id)
                        @endcomponent
                        ...
                        @component('dashboard._components._form-group.input-radio')
                            ...
                            @slot('inputValue', $product->status)
                            @slot('inputDatas', [
            ...
            document.getElementById('input-photo').addEventListener('change', function(e) {
                ...
                if (file && file.size > maxSize) {
                    ...
                    document.getElementById('img-product').src =
                        '{{ $product->photo ? asset('storage/' . $product->photo) : '' }}'; // Reset to original
                }
            });
        </script>
    @endpush
